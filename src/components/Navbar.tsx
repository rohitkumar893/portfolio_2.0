  ...
  return (
    <section className={`navbar border-b-3 border-b-gray-200 fixed select-none top-0 left-0 h-16 sm:h-20 w-full flex justify-between px-4 sm:px-6 md:px-8 lg:px-12 items-center text-lg font-normal text-white z-50 transition-all duration-300`}>
      <a href="#home" className="name flex justify-center items-center gap-2 sm:gap-2.5 group">
        ...
        <h1 className="rohittext text-2xl sm:text-3xl md:text-4xl text-gray-200 font-bold bg-clip-text transition-all duration-300 group-hover:scale-105">Rohit.</h1>
      </a>
      
      {/* Hamburger Button */}
      <button 
        className='lg:hidden z-[60] relative p-2 rounded-lg hover:bg-gray-800/50 transition-colors duration-200'
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`mobile-menu-overlay fixed top-0 left-0 w-full h-screen overflow-y-auto bg-[#141010]/98 backdrop-blur-xl z-[55] transition-all duration-300 ${
          isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
        }`}
        onClick={() => setIsOpen(false)}
      >
        <nav className='flex flex-col gap-4 sm:gap-6 px-5 sm:px-8 pt-20 sm:pt-32 text-lg sm:text-2xl text-white min-h-full'>
          <a 
            href="#home" 
            onClick={() => setIsOpen(false)}
